...
class H36M_AL(torch.utils.data.Dataset):
    ...
    (2) 
    '''
    def __init__(self, h36m_dict, activelearning_obj, getitem_dump, conf, **kwargs):
        '''
        '''
        self.conf = conf
    ...
    def train_model_3D(self):
        ...
        for e in range(self.start_epoch, self.epochs_3D):
            ...
            for (inp, out, meta, images) in tqdm(self.torch_dataloader):
                ...
                loss_func = criterion(pred_3D, target_3D).mean()
    ...
    def inference(self):
        ...
        for i, batch in enumerate(tqdm(self.dataset_obj)):
            with torch.no_grad():
                ...
                exit()
                pred_3D, mu, log_var = self.network_3D.forward(torch.tensor(target_3D, device=opt.device), torch.tensor(pred_val, device=opt.device))
                b_size = 1
                pred_3D = torch.reshape(pred_3D, (b_size, 16, 3))
                
                print(pred_3D)
                print('GT')
                print(target_3D)
                mpjpe_rec = mpjpe(pred_3D, target_3D).item() * 1000
...
def main(opt,conf):
    ...
    if conf.demo:
        ...
        h36m = load_h36m(opt, 'train', precached_h36m = conf.precached_h36m)
        #print('main_h36m: ', h36m.keys()) #(['input', 'target', 'meta', 'reg_target', 'reg_ind', 'reg_mask', 'image'])
        h36m_Data = H36M_AL(h36m_dict = h36m, activelearning_obj = active_learning_obj, getitem_dump =conf.model_save_path, conf = conf, **args)
        print("Done with active learning dataset")

        torch_dataloader = torch.utils.data.DataLoader(h36m_Data , batch_size=32,
