...
def load_h36m(opt, split, precached_h36m=False):
    ...
    if precached_h36m:
      ...
      h36m_dict = np.load('../../../../../../data/tmp/GO/h36m_cache_{}_15000.npy'.format(split), allow_pickle=True)
      h36m_dict = h36m_dict[()]
    ...
    image_path =  os.path.join(opt.data_dir, 'h36m', 'ECCV18_Challenge', 'ECCV18_Challenge', split_, 'IMG') # change
    ...
    for img_idx in tqdm(range(15000)): #num_samples
        try:
    ...
    np.save(file=os.path.join('h36m_cache_{}_15000.npy'.format(split)),
            arr=h36m_dict,
            allow_pickle=True)
