...
from train_2D import Train2

# 0606 mpi_inf_3dhp dataset
...
# 0622 mpi3d dataset
# from mpi_inf_3dhp_dataloader import mpi_inf_3dhp_AL
...
def train_model_3D_simple(train_loader, network_2D, network_3D, network_ll, criterion, optimizer, 
    ...
    mpi3d_index =  [14, 8, 9, 10, 11, 12, 13, 15, 1, 16, 0, 5, 6, 7, 2, 3, 4]
    ...
    # if conf.mpi_inf_3dhp:

    #     for batch in tqdm(train_loader):

    #         input_2D = batch['input'].permute(0, 2, 3, 1)
    #         input_2D = input_2D.cuda(non_blocking=True)
    #         target_3D = batch['target']
    ...
    for (inp, out, meta, images) in tqdm(train_loader):    
        ...
        if conf.mpi_inf_3dhp: 
            target_3D = meta[:, :, :3].cuda()
            #print("target_3D: ", target_3D.shape)
            target_3D = target_3D[:, mpi3d_index, :].float()
        else:
            target_3D = meta['gt_3d'].cuda() #x 
        #print("check target_3D: ", target_3D.shape)
        ...
        if conf.train_3D:
            ...
            #print("check outputs: ", outputs.shape)
        ...
        # print("check outputs type:", outputs.dtype)
        # print("check target type:", target_3D.dtype)
...
def main(opt,conf):
    ...
    if conf.mpi_inf_3dhp:
        ...
        """
        dict_keys(['frame_ref', 'index', 'valid_depth', 'original_skel', 'camera_intrinsic', 'camera_extrinsic', 'target', 'transform_opts', 'joint_mask', 'input'])

        """
        mpi_3d_dataset_length = len(mpi_3d_dataset)
        print("check # of mpi3d: ", mpi_3d_dataset_length)

        # 存成dict
        # 先存成dict後
        # 在往下call __init__.py裡面的MPI3D_AL
        # torch.tensor(self.input_to_tensor(img)), frame_ref_id, target, index
        # mpi3d_dict = {'inp': [], 'out': [], 'target': [], 'index':[]}
        # for (inp, out, target, index) in mpi_3d_dataset:
        #     mpi3d_dict['inp'].append(inp)
        #     mpi3d_dict['out'].append(out)
        #     mpi3d_dict['target'].append(target)
        #     mpi3d_dict['index'].append(index)

        print("check with # of mpi3d: ", len(mpi3d_dict['index']))
        # np.save('mpi3d.npy', arr=mpi3d_dict, allow_pickle=True)
        ...
        split_totoal_indices = list(range(mpi_3d_dataset_length))
        val_ratio = 0.2
        random_seed = 42

        split = int(np.floor(val_ratio*mpi_3d_dataset_length))
        ...
        # data_dict = mpi_3d_dataset.__getitem__(2000)
        ...
        # dataset return 9 items

        torch_dataloader = torch.utils.data.DataLoader(mpi_3d_dataset , batch_size=32, pin_memory=True,
                                                                 num_workers=8, drop_last=True, sampler = train_sampler) #shuffle=True,
        val_loader = torch.utils.data.DataLoader(mpi_3d_dataset, batch_size=32, pin_memory=True,
                                                                 num_workers=8, drop_last=True, sampler = valid_sampler) 

        print("len of torch_dataloader(mpi3d): ", len(torch_dataloader))
        print("len of val_dataloader(mpi3d): ", len(val_loader))

        # print(mpi_3d_dataset[2000])
        # for (idx, target) in tqdm(torch_dataloader):
        #     print(idx)
        #     print(target)
        #     exit()
        # 成功!
        ...
        #dict_keys(['frame_ref', 'index', 'valid_depth', 'original_skel', 'camera_extrinsic', 'target', 'transform_opts', 'joint_mask', 'input'])
        

        #for (meta, index, _, _, _, targt, _, _, img) in next(iter(torch_dataloader):    
            #print(meta)

            #exit()
        
        
        print("Done with mpi3d dataset")
