'''
Ref: h36m_dataset.py from Calvin
'''
...
import copy
import torch
# 看一下camera.py and skeleton.py(都在utils folder)
...
class Human36mDataset(MocapDataset):
    ...
    """
    def __init__(self, path_2d, path_3d, is_train=True):
        h36m_skeleton = Skeleton(parents=[-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14, 12,
                                          16, 17, 18, 19, 20, 19, 22, 12, 24, 25, 26, 27, 28, 27, 30],
                                 joints_left=[6, 7, 8, 9, 10, 16, 17, 18, 19, 20, 21, 22, 23],
                                 joints_right=[1, 2, 3, 4, 5, 24, 25, 26, 27, 28, 29, 30, 31]) # total: 26 joints?
            ...
            for i, cam in enumerate(cameras):
                ...
                for k, v in cam.items():
                    if k not in ['id', 'res_w', 'res_h']:
                        cam[k] = np.array(v, dtype='float32') # 將除了id, res_w, res_h 以外的全都轉為float32
...
import copy
# from utils.skeleton import Skeleton
# from dataloader.mocap_dataset import MocapDataset # 原來code有但因為我把他丟在同一個dataloader_3D.py所以
# from utils.camera import * 有了utils_3D.py 
import torch
...
class Human36mDataset(MocapDataset):
    ...
    """
    predicted 2D: laod predict 完的2D positions/corridinates?

    """

    '''
    condition = predicted 2D: position_2d
    gt_2d use image_coordinates
    '''
    def __init__(self, path_2d, path_3d, is_train=True):
        h36m_skeleton = Skeleton(parents=[-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14, 12,
                                          16, 17, 18, 19, 20, 19, 22, 12, 24, 25, 26, 27, 28, 27, 30],
        ...
        data = np.load(self.path_3d, allow_pickle=True)['positions_3d'].item()
        _3d_data = {}
        for subject, actions in data.items():
            _3d_data[subject] = {}
            for action_name, positions in actions.items():
                _3d_data[subject][action_name] = {
                    'position_3d_world': torch.from_numpy(positions).float(),
                    'camera': self._cameras[subject],
                    'action': action_name,
                    'subject': subject
                }


        keypoints = np.load(self.path_2d, allow_pickle=True)
        #print(keypoints['metadata'].item()) #{'layout_name': 'h36m', 'num_joints': 17, 'keypoints_symmetry': [[4, 5, 6, 11, 12, 13], [1, 2, 3, 14, 15, 16]]}
        self._keypoints_metadata = keypoints['metadata'].item()
        keypoints = keypoints['positions_2d'].item() # keypoints is dicts
        #print(keypoints.keys()) # dict_keys(['S6', 'S7', 'S5', 'S11', 'S1', 'S9', 'S8', 'S2', 'S3', 'S4'])
        
        if is_train:
            db_subjects = TRAIN_SUBJECTS
        else:
            db_subjects = TEST_SUBJECTS

        self._data = []
        
        kept_joints = self._skeleton.remove_joints([4, 5, 9, 10, 11, 16, 20, 21, 22, 23, 24, 28, 29, 30, 31])

        for subject, actions in data.items():
            assert subject in keypoints, 'Subject {} is missing from the 2D detections dataset'.format(subject)
            if subject in db_subjects:
                for action, positions in actions.items():
                    assert action in keypoints[subject], 'Action {} of subject {} is missing from the 2D detections dataset'.format(action, subject)
                    if 'position_3d_world' not in _3d_data[subject][action]:
                        continue
                    
                    for cam_idx in range(len(keypoints[subject][action])):
                        # We check for >= instead of == because some videos in H3.6M contain extra frames
                        mocap_length = _3d_data[subject][action]['position_3d_world'].shape[0]

                        if keypoints[subject][action][cam_idx].shape[0] > mocap_length:
                            # Shorten sequence
                            keypoints[subject][action][cam_idx] = keypoints[subject][action][cam_idx][:mocap_length]
                        
                        for i in range(0, mocap_length):
                            data = _3d_data[subject][action].copy()
                            res_w = data['camera'][cam_idx]['res_w']
                            res_h = data['camera'][cam_idx]['res_h']
                            data['position_3d_world'] = data['position_3d_world'][i][kept_joints]
                            data['viz_camera'] = cam_idx
                            data.update({'position_3d': world_to_camera(data['position_3d_world'], data['camera'][cam_idx]['orientation'], data['camera'][cam_idx]['translation'])})
                            data.update({'position_2d': torch.from_numpy(normalize_screen_coordinates(keypoints[subject][action][cam_idx][i], w=res_w, h=res_h)).float()})
