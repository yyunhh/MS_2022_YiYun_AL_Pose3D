...
class Train_3D(object):
    ...
    def train_model_3D(self):
        ...
        # Initial parameters for 3D model
        ...
        for e in range(self.start_epoch, self.epochs_3D):
            ...
            for (inp, out, meta, images) in tqdm(self.torch_dataloader):
                ...
                loss_func = criterion(pred_3D, target_3D)
                #print(loss_func.shape) #32, 17, 3


                mpjpe_train_3D = mpjpe(pred_3D, target_3D).item()
                mpjpe_record_train.append(mpjpe_train_3D)
                ...
                #print(" train_record starts:")
                train_record["total_loss"].append(total_loss_list)
                #train_record["loss_func"].append(loss_func_list) 0208
                #train_record["kl_loss"].append(kl_loss_list)0208
                #print(" train_record end")
            
            # if e%5==0:
            #     print("0225 check 2D input index: ", meta['index'][0])
            #     print("0225 check 2D input: ", pred_2D_train.shape, pred_2D_train[0])
            #     print('0225 check 3D prediction: ', pred_3D.shape, pred_3D[0]) #[4, 16, 3]
            #     print('0225 check 3D target: ', target_3D.shape, target_3D[0])

            # if e%10==0:
            #     print("=======")
            #     print("Inference to see the result: ")
                
            #     print("=======")

            #print("epoch_loss mean starts:")
            #np.save("0328.npy", train_record['total_loss']) 
            #print(type(train_record['total_loss']))
            ...
            f_merge = open(self.model_save_path.format("3D_cVAE_log.txt"), "w")
            f_merge.write('\n'.join([str(mpjpe_mean_train)]))
            f_merge.close()
            # 0422
            ...
            print("done with epoch_loss mean")
            print("tb_writer starts:")
            self.tb_writer.add_scalar('Train', torch.Tensor([epoch_loss]), global_step)
            print("done with tb_writer")

            torch.save(self.network_3D.state_dict(), self.model_save_path.format("last_3D_model.pth")) #0422 add last model

            # 0203 寫validation loss:
            print("validation_3D starts:")

            validation_loss_hg = self.validation_3D(e)
            validation_learning_loss = 0 # 0203 不太清楚這啥
    ...
    def validation_3D(self, e):
        ...
        with torch.no_grad():
            ...
            for (inp, out, meta, images) in tqdm(self.val_dataset_obj):
                ...
                # if self.train_learning_loss: #0203 要寫learnloss 但是我不太懂資料跑去哪
...
def main(opt,conf):
    ...
    optimizer_3D_simple = torch.optim.Adam(model_3D_simple.parameters(), lr=1.0e-3) #lr=opt.lr # 0420

    # logger_3D_simple = log.Logger(os.path.join('checkpoint/', '3D_simple_log.txt'))
    
    logger_3D_simple = log.Logger(os.path.join(conf.model_save_path[:-20], 'checkpoint/3D_simple_log.txt'))
    logger_3D_simple.set_names(['epoch', 'lr', 'loss_train', 'loss_test', 'mpjpe_mean(train)', 'mpjpe_std(train)', 'mpjpe_mean(test)', 'mpjpe_std(test)'])
    ...
    if conf.train_3D_simple:
        ...
        def train_model_3D_simple(train_loader, network_2D, network_3D, criterion, optimizer, 
            ...
            #path_log = os.path.join('checkpoint/', 'prediction.txt')
            ...
            for (inp, out, meta, images) in tqdm(train_loader):    
                ...
                #print("meta info ", meta.keys()) #meta info  dict_keys(['index', 'center', 'scale', 'gt_3d', 'pts_crop', 'normalizer'])


                # print("meta info ", meta['index'][0])
                # print("meta info (gt_3d) ", meta['gt_3d'][0])
                # print("2D prediction: ", pred_2D_train[0])
                
                # print("3D GT(info): ", target_3D[0])
                ...
                #0421 end
                outputs = network_3D(pred_2D_train) # 0421 open later since train with gt_2D
                # modify
                ...
                #     f.write("\n".join([str(meta['index'][0])]))
                #     f.write("\n".join([str(pred_2D_train[0])]))
                #     f.write("\n".join([str(outputs[0])]))
                #     f.write("\n".join([str(target_3D[0])]))
            ...
            bar.finish()
            print(lr_now)
            return glob_step, lr_now, losses_simple.avg, mpjpe_mean_train, mpjpe_std_train
        ...
        def test_model_3D_simple(test_loader, network_2D, network_3D, criterion):
            ...
            bar.finish()
            return losses.avg, mpjpe_mean_test, mpjpe_std_test
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            # exit()
            glob_step, lr_now, loss_train, mpjpe_mean_train, mpjpe_std_train= train_model_3D_simple(
                torch_dataloader, network_2D = model, network_3D = model_3D_simple, criterion = criterion_3D_simple, optimizer = optimizer_3D_simple,
                lr_init=lr_init, lr_now=lr_now, glob_step=glob_step, lr_decay=lr_decay, gamma=lr_gamma,
                max_norm=True)

            loss_test, mpjpe_mean_test, mpjpe_std_test = test_model_3D_simple(val_loader, network_2D = model, network_3D = model_3D_simple, criterion = criterion_3D_simple)
            
            print(epoch + 1, lr_now, loss_train, loss_test, loss_train)
            # update log file
            logger_3D_simple.append([epoch + 1, lr_now, loss_train, loss_test, mpjpe_mean_train, mpjpe_std_train, mpjpe_mean_test, mpjpe_std_test],
                        ['int', 'float', 'float', 'flaot', 'float', 'float', 'float', 'float'])
