...
## add
def show_2d_2(img, points, c, edges, gt, c2):
  num_joints = points.shape[0]
  points = ((points.reshape(num_joints, -1))).astype(np.int32)
  for j in range(num_joints):
    cv2.circle(img, (points[j, 0], points[j, 1]), 3, c, -1)
    cv2.circle(img, (gt[j, 0], gt[j, 1]), 3, c2, -1)
  ...
  for e in edges:
    if points[e].min() > 0:
      cv2.line(img, (points[e[0], 0], points[e[0], 1]),
                    (points[e[1], 0], points[e[1], 1]), c, 2)
      cv2.line(img, (gt[e[0], 0], gt[e[0], 1]),
                    (gt[e[1], 0], gt[e[1], 1]), c2, 2)
  return img
...
class Debugger(object):
  ...
  def add_point_2d_2(self, point, c, imgId='default', gtt=None, c2=None):
    self.imgs[imgId] = show_2d_2(self.imgs[imgId], point, c, self.edges, gtt, c2)
